...
module.exports = {
  truncate: truncate,
  generateComponentPosts: generateComponentPosts,
};
...
async function generateComponentPosts(
  componentDir,
  {siteConfig, siteDir},
  options,
) {
  ...
  const componentFiles = await globby(include, {
    cwd: componentDir,
  });
  ...
  await Promise.all(
    componentFiles.map(async (relativeSource) => {
      const source = path.join(componentDir, relativeSource);
      const aliasedSource = aliasedSitePath(source, siteDir);
      const componentFileName = path.basename(relativeSource);
      ...
      const match = componentFileName.match(FILENAME_PATTERN);
      let linkName = componentFileName.replace(/\.mdx?$/, '');
      if (match) {
      ...
      // Use file create time for component.
